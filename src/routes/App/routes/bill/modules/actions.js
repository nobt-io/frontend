...
export const focusIdChanged = focusId =>
  createAction(FOCUS_ID_CHANGED, { focusId });
export const newMemberAdded = member =>
  createAction(NEW_MEMBER_ADDED, { member });
export const newDebteeSelected = debtee =>
  createAction(NEW_DEBTEE_SELECTED, { debtee });

export const saveConversionInformation = payload =>
  createAction(SAVE_CONVERSION_INFORMATION, payload);
export const clearConversionInformation = () =>
  createAction(CLEAR_CONVERSION_INFORMATION);

export const addBillFailed = error =>
  createAction(UPDATE_ADD_BILL_STATUS, {
    error,
    status: AsyncActionStatus.FAILED,
  });
export const addBillSucceeded = response =>
  createAction(UPDATE_ADD_BILL_STATUS, {
    response,
    status: AsyncActionStatus.SUCCESSFUL,
  });
export const addBillStarted = () =>
  createAction(UPDATE_ADD_BILL_STATUS, {
...
export const descriptionChanged = description =>
  createAction(DESCRIPTION_CHANGED, { description });
export const shareValueChanged = (name, value) =>
  createAction(SHARE_VALUE_CHANGED, { name, value });
