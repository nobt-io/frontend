...
export const getDebtee = createSelector(
  [getAddBillFormSlice],
  state => state.debtee
);
...
export const getFocusId = createSelector(
  [getAddBillFormSlice],
  state => state.focusId
);
export const getAddBillStatus = createSelector(
  [getAddBillFormSlice],
...
export const getAllMembers = createSelector(
  ...
  (billMembers, nobtMembers, debtee) => {
    return [...new Set([...billMembers, ...nobtMembers, debtee])]
      .filter(name => name !== null)
      .sort(personNameComparator);
  }
);
...
const getDefaultValueForSplitStrategy = createSelector(
  [getSplitStrategy, getDefaultValues],
  (splitStrategy, defaultValues) => defaultValues[splitStrategy]
);

export const isForeignCurrencyBill = createSelector(
  [getAddBillFormSlice],
  state => {
...
export const getConversionInformation = createSelector(
  [getAddBillFormSlice],
  state => {
    return state.conversionInformation;
  }
...
export const getPersonValues = createSelector(
  ...
  (state, defaultValue, members, billMembers) => {
    ...
    let defaultPersonValues = members
      .filter(name => !hasExplicitPersonValue(name))
      .map(createPersonValue);
...
export const areAllMembersSelected = createSelector(
  [getPersonValues],
  personValues => {
...
const getEqualShares = createSelector(
  ...
  (amount, personValues, members) => {
    const noShare = name => {
      return { name: name, amount: null, value: false };
    };

    let involvedMembers = personValues
    ...
    const regularShare = name => {
      return { name: name, amount: share, value: true };
    };

    const isInvolved = name => involvedMembers.indexOf(name) !== -1;
    const mapFn = name =>
      isInvolved(name) ? regularShare(name) : noShare(name);
...
const getPercentualShares = createSelector(
  ...
  (amount, personValues, members) => {
    const noShare = name => {
      return { name: name, amount: null, value: 0 };
    };

    var involvedMembers = personValues
    ...
    if (sumOfPercentages === 100) {
      const roundingError =
        amount - shares.reduce((sum, c) => sum + c.amount, 0) || 0;
...
const didAddBillFail = createSelector(
  [getAddBillStatus],
  status => status === AsyncActionStatus.FAILED
);

export const isDescriptionErrorShown = createSelector(
  [didAddBillFail, isDescriptionValid],
  (failed, valid) => failed && !valid
);
export const isAmountErrorShown = createSelector(
  [didAddBillFail, isAmountValid],
  (failed, valid) => failed && !valid
);
export const isDebteeErrorShown = createSelector(
  [didAddBillFail, isDebteeValid],
  (failed, valid) => failed && !valid
);
export const isDebtorsSelectionErrorShown = createSelector(
  [didAddBillFail, isDebtorsSelectionValid],
  (failed, valid) => failed && !valid
);

export const isExistingMemberFactory = createSelector(
  [getAllMembers],
  members => {
